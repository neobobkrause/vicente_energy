...
class ForecastSolarService(ForecastService):
    ...
    def _handle_current_hour_change(self, entity_id, old_state, new_state) -> bool:
        try:
            ...
            _LOGGER.warning("Failed to parse Forecast.Solar current hour forecast from state: %s", new_state.state)
            return False
    ...
    def _handle_next_hour_change(self, entity_id, old_state, new_state) -> bool:
        try:
            ...
            _LOGGER.warning("Failed to parse Forecast.Solar next hour forecast from state: %s", new_state.state)
            return False
    ...
    def _handle_now_production_change(self, entity_id, old_state, new_state) -> bool:
        try:
            ...
            _LOGGER.warning("Failed to parse Forecast.Solar production now from state: %s", new_state.state)
            return False
