...
from .services import SERVICE_CLASS_MAP
...
class VicenteEnergyConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None) -> FlowResult:
        ...
        loaded_integrations: set[str] = async_get_loaded_integrations(self.hass)
        ...
        schema_fields = {
            ...
            vol.Required(CONF_RESERVE_SOC_PCT, default=20): vol.All( # type: ignore
                vol.Coerce(int),
                vol.Range(min=0, max=100)
            ),
            vol.Required(CONF_STORAGE_EFFICIENCY, default=90): vol.All( # type: ignore
                vol.Coerce(float),
        ...
        if user_input is not None:
            ...
            if any(entry.data.get(CONF_LOCATION_NAME) == location_name
